...
void Path_GetExecutablePath(char* buf, size_t sz)
{
	int bSuccess = 0;
#if defined( _WIN32 )
	bSuccess = GetModuleFileNameA(NULL, buf, (DWORD)sz) > 0;
// #elif defined OSX
...
{
	char slash = Path_GetSlash();

	char* slash_pos = strrchr(in, slash);
	if (slash_pos)
	{
		strncpy_s(out, szOut, in, slash_pos - in);
	}
	else
	{
		out[0] = '\0';
...
void Path_MakeAbsolute(const char* sRelativePath, const char* sBasePath, char* out, size_t sz)
{
	...
	else
	{
		...
		Path_Join(sBasePath, sRelativePath, out, sz);
		Path_Compact(out, sz);
		if (!Path_IsAbsolute(out))
			out[0] = '\0';
	}
}
...
void Path_FixSlashes(char* inout, size_t sz)
{
	...
	for (size_t i = 0; i < sz && inout[i] != '\0'; ++i)
	{
		if (inout[i] == '/' || inout[i] == '\\')
			inout[i] = slash;
	}
...
void Path_Join(const char* first, const char* second, char* out, size_t sz)
{
	...
	size_t nLen = strlen(first);
	char last_char = first[nLen - 1];
	if (last_char == '\\' || last_char == '/')
		--nLen;

	strncpy_s(out, sz, first, nLen);
	strncat_s(out, sz, &slash, 1);
	strcat_s(out, sz, second);
}
...
// 	}
	...
	size_t i = 0;
	while (i < strlen(inout))
	{
		if (i > 0 && strlen(inout) - i >= 2
			&& inout[i] == '.'
			&& inout[i + 1] == '.'
			&& (i + 2 == strlen(inout) || inout[i + 2] == slash)
			&& inout[i - 1] == slash)
		{
			...
			size_t iDirStart = i - 2;
			while (iDirStart > 0 && inout[iDirStart - 1] != slash)
				--iDirStart;

			// remove everything from iDirStart to i+2
			memmove(inout + iDirStart, inout + i + 3, strlen(inout) - (i + 3) + 1);
